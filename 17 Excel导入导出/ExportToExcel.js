...
export class ExportExcel {
    ...
    /**
     * @desc    将数据表格数据导出为excel表格
     * @param tableData 需要导出的数据源
     * @param tableHeader 指定表头，格式：{name:'名字',prop:'name'} or {name:'信息',child:[{name:'名字',prop:'name'}]}
     * @param name  指定导出文件的名字
     */
    exportData (tableData, tableHeader, name) {
        if (!Array.isArray(tableData) || tableData.length < 1) {
            throw { type: 'error', message: '请选择需要导出的数据！' }
        }
        let sheetName = name ? name : 'xlsx复杂表格导出demo'
        // excel表头
